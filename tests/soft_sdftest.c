...
size_t test_sm3_crypt_loop(size_t size){
    ...
    for (count = 0;  run&count < 0xffffffffffffffff; count++)
    {
		...
		ret = SDF_HashUpdate(hSessionHandle, test_input, size);
		if (ret != SDR_OK) {
			fprintf(stderr, "Error: SDF_HashUpdate: 0x%X\n", ret);
			return -1;
		}
...
static int SDF_TEST_SM2_PERFTEST_SPEED(void){
	size_t size=48;
	printf("SDF_InternalDecrypt_ECC_Enc_and_Dec:\n");
	performance_test_sv(test_SDF_InternalDecrypt_ECC_loop,test_SDF_InternalDecrypt_ECC_loop,&size,1,10);
...
int main(void)
{
	...
	if (SDF_TEST_SM2_PERFTEST_SPEED() != 1) goto err;
